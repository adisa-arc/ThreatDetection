#
# -------------------------------------------------------------------------------------------------
# This is the TCPD to JSON RPC tool.
# Decsription:     It reads TCPDs event encoded in SYSLOG messages and converts then to JSON.
...
#
def JSONTCPD(data, dtime, eventid, syslogid):
    ...
    jsonevent = jsonevent + "\"_type\": \"tcpd\","
    ...
    jsonevent = jsonevent + " }, "
    #
    jsonevent = jsonevent + "\"tcpd\": { "
    jsonevent = jsonevent + "\"datetime\": { "
    ...
    jsonevent = jsonevent + "\"message\": { "
    jsonevent = jsonevent + "\"_type\": \"ascii\","
    jsonevent = jsonevent + "\"__text\": \"" + arraytostr(data[7:len(data)]) + "\""
    jsonevent = jsonevent + " }, "

    jsonevent = jsonevent + "\"_datetime\": \"" + dtime + "\", "
    jsonevent = jsonevent + "\"_ident\": \""+ str(digest) + "\", "
    jsonevent = jsonevent + "\"_type\": \"tcpd\""
    jsonevent = jsonevent + " } } ,"
    jsonevent = jsonevent + "\"_datetime\": \"" + dtime + "\", "
...
def syslogParsetoJSON(inputline):
    ...
    #
    if ("in." in data[4][0:3]):
        # -> Process as a TCPD Message
        jsonevent = JSONTCPD(data, dtime, eventid, syslogid)
    else:
...
#
def help():
    print('TCPD to JSON Converter [tcpdjsonrpc] - Version 1.0 - andrew.blyth@adisa.global\n')
    print('USAGE: tcpdjsonrpc [-n number] /path/filename\n') 
    print('WHERE:')
    print('      -n number                       - Specifies the number of entries to process.\n')
    print('Examples of Usage:\n')
    print('       tcpdjsonrpc tcpddata.dat       - This will read and process all enties for ever.\n')
    print('       tcpdjsonrpc -n 8 tcpddata.dat  - This will read and process the first 8 entries.\n')
#
#
#
def main(): 
    ...
    if (len(sys.argv) == 4):
        ...
        print('TCPD to JSON Converter [vsftpfsonrpc] - Version 1.0 - andrew.blyth@adisa.global\n')
        print(":- ERROR: Insufficent command line arguments.\n\n")
        print(len(sys.argv))
        help()
        sys.exit()
